...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const data = await getIntervalPageData(locale, 'tabata')
  return buildPageMetadata(locale, { ...data.metadata, path: data.path })
}

export default async function TabataTimerPage({ params }: PageProps) {
  const { locale } = await params
  setRequestLocale(locale)
  return <IntervalTimerPageContent locale={locale} variant="tabata" />
}
